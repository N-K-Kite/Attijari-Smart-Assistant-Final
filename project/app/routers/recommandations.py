...
def charger_knn() -> None:
    global KNN_MODEL
    # Multiple paths to ensure loading works in all environments
    paths = [
    ...
    for path in paths:
        if os.path.exists(path):
            try:
                ...
                logger.info("✅ SUCCESS: KNN loaded with {} tickets from {}", len(KNN_MODEL["df"]), path)
                return
            except Exception as e:
                logger.error("❌ ERROR loading KNN from {}: {}", path, e)
    
    logger.warning("🚫 KNN NOT FOUND — using basic recommendations")
