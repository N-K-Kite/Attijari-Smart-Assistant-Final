...
## Nouvelles Fonctionnalités (Sprint 1+)

### 📬 Boîte de Réclamations (Inbox)
Le dashboard inclut désormais un système de gestion de tickets complet :
- **Inbox moderne** : Liste des tickets avec recherche et détails (utilisateur, email, description complète).
- **Résolution IA** : Toggle pour marquer les tickets comme "Résolus" avec persistance automatique.
- **Notifications Email** : Envoi automatique à `responsableit000@gmail.com` lors de chaque création de ticket.

### 📧 Configuration Email
Pour que les notifications fonctionnent sur une nouvelle machine :
1. Activez la **validation en 2 étapes** sur le compte Gmail.
2. Créez un **Mot de passe d'application** (App Password).
3. Remplissez `SMTP_PASSWORD` dans le fichier `.env`.
