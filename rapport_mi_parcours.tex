...
La solution développée repose sur une architecture complète couvrant l'ensemble de la chaîne de valeur~: un backend Python intégrant des techniques avancées de traitement du langage naturel (NLP), d'apprentissage profond (LSTM) et d'automatisation robotisée des processus (RPA), et un frontend développé en HTML, CSS et JavaScript offrant aux équipes opérationnelles une interface web intuitive, sécurisée et accessible.
...
L'architecture de développement repose sur une approche multi-couche évolutive, conçue pour évoluer progressivement d'un prototype fonctionnel vers une architecture industrielle~:
...
\hline
Frontend (Prototype) & HTML5 · CSS3 · JS & Rapidité de prototypage, validation des maquettes \\
\hline
Frontend (Production) & Angular 21 · TypeScript & Architecture SPA modulaire, routing avancé \\
\hline
Backend (Prototype) & PHP 8 · Sessions & Déploiement rapide sur XAMPP \\
\hline
Backend (Production) & Spring Boot 3.4 · JPA & Robustesse, écosystème Java, JWT natif \\
\hline
Backend IA & Python · FastAPI & Pipeline NLP, modèles IA, API REST \\
\hline
Base de données & MySQL (InnoDB) & Support transactionnel, contraintes FK \\
\hline
Design System & Inter · CSS Variables & Identité visuelle premium Attijari \\
\hline
\end{tabularx}
\end{table}

\section{Développement du Prototype Frontend}

Le prototype initial a été développé en HTML5, CSS3 et JavaScript Vanilla afin de valider rapidement les maquettes d'interface avec l'encadrant bancaire. Ce prototype comprend \textbf{six pages complètes}.
...
\begin{itemize}[noitemsep]
    \item \textbf{Section Hero} avec animations dynamiques (orbes, grille de fond, gradient text), statistiques clés (70\% d'automatisation, disponibilité 24/7, -45\% volume d'appels)
    \item \textbf{Slider interactif} à 3 slides avec navigation par flèches et indicateurs à points~: présentation du projet, mockup mobile, et service client virtuel
    \item \textbf{Grille de 6 fonctionnalités}~: Chatbot Intelligent, Routage Intelligent, Tableau de Bord, Disponibilité 24/7, Sécurité Bancaire, Intégration API
    \item \textbf{Section processus} illustrant le flux en 4 étapes~: Contact Client $\rightarrow$ Analyse NLP $\rightarrow$ Résolution Auto $\rightarrow$ Escalade Agent
...
Le chatbot constitue le composant central de l'interface utilisateur, développé sous forme d'une \textbf{classe JavaScript orientée objet} (\texttt{AttijariChat}) intégrant~:

\begin{itemize}[noitemsep]
    \item \textbf{Moteur de détection d'intention}~: classification par correspondance de patterns sur 8 catégories (Solde, Blocage, Transaction, Réclamation, Agence, Agent, Crédit, Salutation), avec calcul d'un score de confiance dynamique
    \item \textbf{Architecture prête pour l'API backend}~: configuration permettant le basculement du mode démonstration vers l'API réelle FastAPI (\texttt{http://localhost:8000/chat}), avec gestion des en-têtes JWT et traitement des erreurs
    ...
    \item \textbf{Panneau de contexte client} (panneau droit)~: affichage en temps réel du profil utilisateur authentifié, intention détectée, analyse de sentiment, barre de confiance colorée
    \item \textbf{Sidebar latérale}~: sessions actives, boutons de demandes rapides (Solde, Bloquer carte, Transactions, Virement, Agences, Agent), indicateurs de statut
    ...
    \item \textbf{Carte d'escalade}~: interface visuelle de transfert vers agent humain avec file d'attente estimée
...
Le dashboard analytique est un composant complexe développé \textbf{entièrement en Canvas 2D natif}, sans aucune bibliothèque externe de graphiques~:

\begin{itemize}[noitemsep]
    \item \textbf{6 cartes KPI animées} avec compteur incrémental~: Total Interactions, Résolus par IA, Escaladés Agent, Satisfaction Client, Temps de Réponse Moyen, Taux d'Automatisation
    \item \textbf{Graphique en courbes} (Line/Area)~: Volume d'Interactions sur 24h, double série (Chatbot IA vs Agents Humains) avec remplissage en gradient
    \item \textbf{Graphique en barres horizontales}~: Top Intentions Détectées (6 catégories triées)
    \item \textbf{Graphique Donut}~: Taux de Résolution IA vs Agents avec valeur centrale dynamique
    \item \textbf{Graphique en barres empilées}~: Analyse de Sentiment sur 7 jours (Positif, Neutre, Négatif)
    \item \textbf{Flux en temps réel} (Live Feed)~: événements toutes les 2,5--4,5 secondes avec indicateur \texttt{LIVE} pulsant
    \item \textbf{Table des agents}~: 5 agents avec statut, file d'attente et compteur de résolutions
    \item \textbf{Filtres temporels} (Aujourd'hui, 7 jours, 30 jours) avec rechargement dynamique
    \item \textbf{Graphiques adaptatifs} au redimensionnement de la fenêtre
\end{itemize}
...
\begin{itemize}[noitemsep]
    \item \textbf{Page de connexion}~: formulaire CIN + mot de passe, appel asynchrone (\texttt{fetch}) vers l'API, gestion des erreurs avec messages visuels, redirection post-connexion
    \item \textbf{Page d'inscription complète}~: formulaire en 3 sections (Informations Personnelles, Données Bancaires, Sécurité) avec validation côté client
    \item \textbf{Module d'authentification partagé} (\texttt{auth.js})~: vérification automatique de session à chaque chargement de page, contrôle d'accès basé sur les rôles (RBAC --- \texttt{admin} pour le dashboard, \texttt{user} pour le chatbot), gestion de déconnexion sécurisée
    \item \textbf{Synchronisation inter-composants}~: événement personnalisé \texttt{userAuthenticated} pour partager l'état d'authentification entre les modules
\end{itemize}

\section{API Backend}

L'API REST a été développée pour assurer les opérations CRUD et l'authentification. Elle comprend 6 endpoints~:
...
\hline
\texttt{/config} & --- & Configuration base de données (PDO), CORS, sessions \\
\hline
\texttt{/login} & POST & Authentification CIN + mot de passe, vérification bcrypt \\
\hline
\texttt{/register} & POST & Inscription avec validation (CIN unique, 8 chiffres, mdp $\geq$ 8 car.) \\
\hline
\texttt{/check\_auth} & GET & Vérification de session active, retour du rôle et profil \\
\hline
\texttt{/get\_user} & GET & Profil complet avec soldes et 5 dernières transactions (JOIN) \\
\hline
\texttt{/logout} & GET & Destruction de session \\
\hline
\end{tabularx}
\end{table}
...
Le schéma de base de données comprend 2 tables principales avec relations et contraintes~:

\begin{itemize}[noitemsep]
    \item \textbf{Table \texttt{users}}~: identifiant CIN unique, informations personnelles, mot de passe haché (bcrypt, coût 12), données bancaires (carte, soldes courant et épargne), rôle (\texttt{user}/\texttt{admin}), métadonnées temporelles
    \item \textbf{Table \texttt{transactions}}~: historique financier lié par clé étrangère \texttt{user\_id}, description, montant (DECIMAL 15,3), marchand, date, type (crédit/débit), suppression en cascade
    \item \textbf{Encodage}~: \texttt{utf8mb4\_unicode\_ci} pour le support complet des caractères arabes et français
    \item \textbf{Données de test}~: 2 utilisateurs (administrateur + utilisateur démo) et 10 transactions
\end{itemize}

\section{Migration vers Architecture Moderne}

En parallèle du prototype fonctionnel, une migration architecturale a été amorcée~:

\subsection{Frontend Angular 21}

Le prototype a été restructuré en Single Page Application (SPA) Angular~:

\begin{itemize}[noitemsep]
    \item 5 composants de pages~: \texttt{HomeComponent}, \texttt{LoginComponent}, \texttt{RegisterComponent}, \texttt{DashboardComponent}, \texttt{ChatComponent}
    \item 2 composants réutilisables~: \texttt{NavbarComponent}, \texttt{FooterComponent}
    \item 2 services injectables~: \texttt{AuthService}, \texttt{ChatService}
    \item Routing complet (\texttt{app.routes.ts}) avec 5 routes et wildcard redirect
\end{itemize}

\subsection{Backend Spring Boot 3.4}

La migration backend vers Java 17 + Spring Boot a été initiée~:

\begin{itemize}[noitemsep]
    \item Spring Boot Starter Web pour l'API REST
    \item Spring Data JPA pour la couche d'accès aux données
    \item Spring Security pour l'authentification
    \item MySQL Connector/J pour la connexion à la base existante
    \item Architecture de packages~: \texttt{com.attijari.backend}
\end{itemize}
...
\hline
Prototype frontend complet (6 pages) & \done & 100\% \\
\hline
...
\hline
Migration Spring Boot amorcée & \inprog & 40\% \\
\hline
\end{tabularx}
\end{table}
...
\hline
Finaliser la migration Angular et Spring Boot & S6--S7 & Moyenne \\
\hline
...
\hline
1 & 23 Mars -- 5 Avril & Prototype frontend conforme Attijari · Création BDD · Nettoyage données · Stockage & Prototype + BDD \\
\hline
...
\subsection{Phase 1 --- Prototype et Données (23 Mars -- 5 Avril) \done}
...
\begin{enumerate}[noitemsep]
    ...
    \item Développer le tableau de bord avec graphiques Canvas natifs et flux en temps réel
    \item Développer les pages de connexion et d'inscription avec validation
    \item Implémenter le module d'authentification RBAC partagé
    \item Concevoir le schéma de la base de données MySQL (2 tables, contraintes FK)
    \item Développer l'API REST (6 endpoints) avec authentification bcrypt
...
\begin{enumerate}[noitemsep, start=15]
    ...
    \item Amorcer la migration du backend vers Spring Boot 3.4
\end{enumerate}
...
\hline
Base de données & MySQL · phpMyAdmin · MySQL Workbench \\
\hline
Build frontend & Angular CLI 21 · npm · TypeScript 5.9 \\
\hline
Build backend & Maven · Spring Boot 3.4 · Java 17 \\
\hline
...
L'ensemble de l'interface web du prototype est fonctionnel et opérationnel~:
...
\hline
\texttt{chat.html} + \texttt{chat.js} & 970 lignes & Chatbot OOP, 8 intents, multi-sessions, contexte \\
\hline
\texttt{dashboard.html} + \texttt{dashboard.js} & 789 lignes & 6 KPI, 4 charts Canvas, live feed, agents \\
\hline
\texttt{login.html} & 182 lignes & Formulaire CIN, API async, gestion erreurs \\
\hline
\texttt{register.html} & 287 lignes & 3 sections, validation, inscription complète \\
\hline
\texttt{auth.js} & 160 lignes & RBAC, session check, événements custom \\
\hline
\texttt{styles.css} & $\sim$1200 lignes & Design system, responsive, animations \\
\hline
API (6 fichiers) & $\sim$500 lignes & CRUD, authentification bcrypt, transactions \\
\hline
\texttt{database.sql} & 86 lignes & Schéma complet, données de test, FK \\
\hline
\textbf{Total V1} & \textbf{$\sim$4\,600} & \textbf{Prototype full-stack fonctionnel} \\
\hline
\end{tabularx}
\end{table}
...
\hline
\texttt{HomeComponent} & \done & Hero + features fidèle au prototype \\
\hline
...
\hline
Pages développées (prototype + Angular) & 6 + 5 = 11 \\
\hline
...
\hline
Graphiques Canvas sans bibliothèque & Complexité élevée pour 4 types de charts en Canvas 2D natif & Développement de fonctions utilitaires réutilisables avec système de coordonnées abstrait \\
\hline
Synchronisation authentification & Le chatbot doit connaître l'utilisateur pour personnaliser les réponses & Système d'événements custom et variable globale partagée \\
\hline
Double migration architecturale & Maintenir le prototype PHP pendant la migration Angular + Spring Boot & Architecture en couches, API PHP conservée comme fallback \\
\hline
...
\hline
Complexité technologique variée & Maîtrise simultanée de JavaScript, Python, Java, SQL & Documentation abondante et apprentissage continu \\
\hline
...
Sur l'axe développement, un \textbf{prototype frontend complet et fonctionnel} a été livré, comprenant 6 pages, un chatbot intelligent avec gestion multi-sessions, un tableau de bord analytique avec 4 graphiques Canvas natifs, un système d'authentification RBAC, et une API REST opérationnelle avec 6 endpoints. La migration vers une architecture moderne (Angular 21 + Spring Boot 3.4) a été amorcée, avec 7 composants Angular et 2 services déjà opérationnels.
...
Les principales difficultés rencontrées --- complexité des graphiques Canvas sans bibliothèque, synchronisation de l'authentification inter-pages, migration double-stack, et ampleur du périmètre technique sur une seule personne --- ont été surmontées par des solutions techniques appropriées et une organisation rigoureuse du travail.
